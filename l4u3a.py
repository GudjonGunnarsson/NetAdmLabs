...
if (len(sys.argv) == 3):
	try:
		...
		try:
			...
			conn = MySQLdb.connect("localhost", "root", "!netadm09#", "my_network")
			#Prepare cursor
			cursor = conn.cursor()
			currId=0
			i=0
			...
			if Model is not None:
				print "Model: "+Model
				#Execute a mysql INSERT for ipAddr, sysName, and Model
				cursor.execute("INSERT INTO unit(ip_address,name,model) VALUES(%s, %s, %s)", (ipAddr,sysName,Model))
			else:
				#Execute a mysql INSERT for ipAddr and sysName
				cursor.execute("INSERT INTO unit (ip_address,name) VALUES (%s, %s)", (ipAddr,sysName))
			#Get the automated ID for this entry
			#currId = cursor.execute("SELECT LAST_INSERT_ID()")
			currId = cursor.lastrowid
			print "Name: "+sysName
			...
			for entries in ifPorts:
				...
				#Execute a mysql INSERT for the values currently being used (current ID, interface type, ifAddress, ifNetmask, and MAC address
				cursor.execute("INSERT INTO interface (unit_id,name,ip_address,mask,mac_address) VALUES (%s, %s, %s, %s, %s)", (currId,port,ifAddr[i],ifNetmask[i],mac))
